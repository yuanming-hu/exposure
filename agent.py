...
def agent_generator(inp, is_train, progress, cfg, high_res=None, alex_in=None):
    ...
    for j, filter in enumerate(filters):
        with tf.variable_scope('filter_%d' % j):
            print('    creating filter:', j, 'name:', str(filter.__class__),
                  'abbr.', filter.get_short_name())
    ...
    with tf.variable_scope('action_selection'):
        ...
        pdf = pdf * (
            1 - cfg.exploration) + cfg.exploration * 1.0 / len(filters)
    ...
    def debugger(debug_info, combined=True):
        ...
        for i, filter in enumerate(filters):
            ...
            selected = i == debug_info['selected_filter_id']
            if selected:
                filter.visualize_filter(debug_info['filter_debug_info'][i],
                                        img)
        if not combined:
            # detail
            images[1] = img.copy()
        ...
        for i, filter in enumerate(filters):
            ...
            cv2.rectangle(img, (corners[0][0] - 1, corners[0][1] - 1),
                          (corners[1][0] + 1,
                           corners[1][1] + 1), (1, 1, 1), cv2.FILLED)
            cv2.rectangle(img, corners[0], corners[1], (color, 0.3, 0.3),
                          cv2.FILLED)
        if not combined:
            # pdf
            images[0] = img.copy()
    ...
    is_last_step = tf.cast(
        tf.abs(states[:, STATE_STEP_DIM:STATE_STEP_DIM + 1] + 1 -
               cfg.test_steps) < 1e-4,
        dtype=tf.float32)
    ...
    early_stop_penalty = (
        1 - is_last_step) * submitted * cfg.early_stop_penalty
