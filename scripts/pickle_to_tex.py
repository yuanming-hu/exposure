...
def visualize_detail(name, param, pos):
    ...
    if name == 'Expo.':
        ...
        scaling = 1 / (
            1e-5 + 0.27 * param[0] + 0.67 * param[1] + 0.06 * param[2])
...
def visualize_step(debug_info, step_name, position):
    ...
    s += visualize_detail(
        filters[filter_id],
        debug_info['filter_debug_info'][filter_id]['filter_parameters'],
        position)
    s += '\n'
    return s
...
def process(filename, id, src='more/'):
    ...
    shutil.copy(src + filename + '.retouched.png',
                target_dir + '/' + 'final.png')
