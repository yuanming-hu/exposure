...
def make_image_grid(images, per_row=8, padding=2):
    ...
    images = np.pad(
        images, pad_width=npad, mode='constant', constant_values=1.0)
    ...
    for j in js:
        ...
        if LIGHTDOWN:
            for _ in range(num - 1):
                out = pow(img, np.random.uniform(
                    0.4, 0.6)) * np.random.uniform(0.25, 0.5)
        ...
        if UNBALANCECOLOR:
            ...
            for _, out in enumerate(
                    filter.process(outs,
                                   filter.filter_param_regressor(features))):
                # print out.max()
                out /= out.max()
...
def vis_images_and_indexs(images, features, dir, name):
    ...
    try:
        ...
        pass
    cv2.imwrite(
        os.path.join(vis_dir, name + '.png'), image[:, :, ::-1] * 255.0)


def read_set(name):
    if name == 'u_test':
        fn = 'folds/FiveK_test.txt'
        need_reverse = False
    elif name == 'u_amt':
        fn = 'folds/FiveK_test_AMT.txt'
        need_reverse = False
    elif name == '5k':  # add by hao
        return list(range(1, 5001))
    elif name == '2k_train':
        fn = 'folds/FiveK_train_first2k.txt'
        need_reverse = False
    elif name == '2k_target':
        fn = 'folds/FiveK_train_second2k.txt'
        need_reverse = False
    else:
