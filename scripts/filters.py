...
class Filter:
    ...
    def apply(self,
        ...
        low_res_output = lerp(img,
                              self.process(img, filter_parameters), self.mask)
        if high_res is not None:
            if self.no_high_res():
    ...
    def get_mask(self, img, mask_parameters):
        ...
        with tf.name_scope(name='mask'):
            ...
            # Strength
            mask = mask * (
                mask_parameters[:, None, None, 5, None] / filter_input_range *
                0.5 + 0.5) * (
            ...
            for j in range(self.cfg.curve_steps):
                p1 = tuple(
                    map(int, (width / self.cfg.curve_steps * j, height - 1 -
                              values[j] * height)))
                p2 = tuple(
                    map(int, (width / self.cfg.curve_steps * (j + 1), height -
                              1 - values[j + 1] * height)))
                color = []
...
class LevelFilter(Filter):
    ...
    def process(self, img, param):
        ...
        return tf.clip_by_value((img - lower) / (upper - lower + 1e-6), 0.0,
                                1.0)
...
class SaturationPlusFilter(Filter):
    ...
    def process(self, img, param):
        ...
        hsv1 = tf.concat(
            [hsv[:, :, :, 0:1], enhanced_s, hsv[:, :, :, 2:]], axis=3)
