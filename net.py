...
class GAN:
    def __init__(self, cfg, restore=False):
        ...
        if cfg.supervised:
            ...
            if cfg.gan == 'ls':
                ...
                self.raw_reward = (
                    cfg.all_reward + (1 - cfg.all_reward) * stopped
        ...
        if cfg.gan == 'ls':
            ...
            self.g_loss = tf.reduce_mean(
                routine_loss +
                self.surrogate_loss_addition * tf.stop_gradient(advantage))
            ...
            self.g_loss = tf.reduce_mean(
                ...
                self.surrogate_loss_addition * tf.stop_gradient(advantage))
            self.emd = -self.c_loss
            self.c_average = tf.reduce_mean(
                self.fake_logit + self.real_logit) * 0.5
        ...
        interpolated = self.real_data + alpha * (
            self.fake_output - self.real_data)
        ...
        gradient_norm = tf.sqrt(1e-6 + tf.reduce_sum(
        ...
        for iter in range(self.cfg.max_iter_step + 1):
            ...
            if iter % 100 == 0:
                ...
                print('# Task: %s  ela. %.2f min  ETA: %.1f/%.1f h' %
                      (self.cfg.name, (time.time() - start_t) / 60.0, eta,
                       tot_time))
                self.memory.debug()
    ...
    def visualize(self, iter):
        ...
        for img, ind in zip(lower_regions[1], best_indices):
            cv2.putText(img,
                        str(ind), (23, 23), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        (1.0, 0.0, 0.0))
        ...
        for i in range(rows):
            for j in range(per_row):
                ...
                index = i * per_row + j
                if index < len(images):
                    upper_region[start_x:start_x + h, start_y:
                                 start_y + w] = images[index]
        ...
        seperator = np.ones(
            (16, upper_region.shape[1], upper_region.shape[2]),
            dtype=np.float32)
    ...
    def eval(self,
        ...
        for fn in spec_files:
            ...
            low_res_img = cv2.resize(
                get_image_center(high_res_image), dsize=(64, 64))
            ...
            for i in range(self.cfg.test_steps):
                ...
                debug_plots = self.generator_debugger(
                    debug_info, combined=False)
            ...
            fused = np.ones(
                shape=(grid * 4, grid * steps, 3), dtype=np.float32)
